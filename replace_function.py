...
# Read the file
with open(r'views\agent\new_group.ejs', 'r', encoding='utf-8') as f:
    content = f.read()

# New function
new_function = '''    // ─── SHOW VERIFICATION FORM ──────────────────────────────
...
pattern = r'    // ─── SHOW VERIFICATION FORM.*?function showVerificationForm\(\) \{.*?\n    \}\n(?=\n    function buildVerificationCard)'

# Replace
new_content = re.sub(pattern, new_function + '\n', content, flags=re.DOTALL)

# Write back
with open(r'views\agent\new_group.ejs', 'w', encoding='utf-8') as f:
    f.write(new_content)
